...
public class RedshiftAutoConfiguration {
    ...
    public RedshiftDataClient redshiftDataClient(
        ...
        var clientBuilder = RedshiftDataClient.builder()
                .region(Region.of(props.getRegion()))
                .credentialsProvider(credentialsProvider);
    ...
    /**
     * Builds an Apache HTTP client with explicit proxy
     * and basic auth credentials.
     *
     * This replicates exactly what AWS CLI does internally
     * when HTTP_PROXY=http://username:password@proxyhost:port
     * is set — ensuring the same proxy path that works in
     * CLI also works in the Java SDK.
     */
    private ApacheHttpClient.Builder buildApacheHttpClient(
        ...
        // Wire up basic auth credentials for proxy
        ...
        var proxyHost = new HttpHost(
                proxy.getHost(),
                proxy.getPort());

        // Build Apache HTTP client with proxy and
        // credentials — same mechanism as AWS CLI
